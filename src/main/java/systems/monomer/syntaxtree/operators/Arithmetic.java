...
class Arithmetic {
    ...
    Operand compileNumericalBinary(AssemblyFile file, GenericOperatorNode self, Runnable ifBothIntegers, Runnable ifFloat) {
        ...
        file.add(Instruction.PUSH, RDX.toOperand(), null)
                .add(Instruction.MOV, first, RDX.toOperand());
        Operand second = self.getSecond().compileValue(file);
        file.add(Instruction.MOV, second, RBX.toOperand());
        ...
        file.add(Instruction.POP, null, RDX.toOperand());
